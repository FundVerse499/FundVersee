...
const MAX_VALUE_SIZE: u32 = 2 * 1024 * 1024; // 2MB per value
...
fn upload_doc(idea_id: u64, name: String, content_type: String, data: Vec<u8>, uploaded_at: u64) -> Option<u64> {
    ...
    if data.len() > 2 * 1024 * 1024 {
        ic_cdk::trap("File size exceeds 2MB limit");
    }
    ...
    DOC_COUNTER.with(|c| {
        ...
        *c += 1;
        ...
        // attach to idea
        IDEAS.with(|ideas| {
            let mut ideas_mut = ideas.borrow_mut();
            if let Some(idea) = ideas_mut.get(&idea_id) {
                let mut idea_clone = idea.clone();
                idea_clone.doc_ids.push(doc_id);
                ideas_mut.insert(idea_id, idea_clone);
            }
        });
